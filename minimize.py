...
#temp_param[10,11,12]   static parameters
#Note that suffixes A,B,C,D below do NOT refer to parameter file peaks A,B,C,D
#Instead, A is active.  It is the peak being fit,while B,C,D are static
...
def center(velocity,intensity,temp_param,startline,endline,steplist):
    ...
    centerD=temp_param[9];coefficientD=temp_param[10];sigmaD=temp_param[11]
    ...
    for i in range(max_passes):
        ...
        for j in range(totallines):
            ...
            exponentD=(-1*(velocity[j]-centerD)**2)/(2*sigmaD**2)
            gaussD=coefficientD*(2.718**exponentD)
        ...
        if i>0 and dir==-1 and sumterms[i]>sumterms[i-1]:
            ...
            sumtermsarray=[0]
            continue
    gaussAA[0]=gaussAA[1];gaussBB[0]=gaussBB[1];gaussCC[0]=gaussCC[1];gaussDD[0]=gaussDD[1];gaussTT[0]=gaussTT[1]   
